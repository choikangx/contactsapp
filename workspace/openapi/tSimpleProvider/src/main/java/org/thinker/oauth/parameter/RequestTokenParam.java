...
//Consumer�� RequestToken ��û�� Consumer�� �����ϴ� ����
...
//		valid = sha1.isValid(OAuthUtil.decode(this.signature), this.baseString);
		...
		if (newSig.equals(this.signature)) 
			...
			valid = false;
		System.out.println("### CallBack URL : " + this.callback);
		...
		if (timeSpan < 0 || timeSpan / 60 > 60 )   //0보다 작다는 미래에서 메세지가 온 경우.
			throw new Exception("timestamp is invalid"); //유효시간 1시간. 
		//타임스탬프는 GMT 기준이라서 해외에 무관하게 동작 가능. 
		//nance 검증이 없네? 우리는 set이 아니라 get만 하니까 없이 하도록 하자. 
